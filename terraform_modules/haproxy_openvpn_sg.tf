# Security group for HAProxy & OpenVPN in the Public Subnet
resource "aws_security_group" "haproxy_openvpn_sg" {
  name        = "haproxy-openvpn-sg"
  vpc_id      = aws_vpc.kubernetesVPC.id
  description = "Security group for HAProxy & OpenVPN instances in the public subnet"
...
resource "aws_vpc_security_group_ingress_rule" "allow_ssh_vpn" {
  security_group_id = aws_security_group.haproxy_openvpn_sg.id
  cidr_ipv4         = "192.168.0.174/32" # Restrict to your static home IP
  from_port         = 22
  to_port           = 22
  ip_protocol       = "tcp"
